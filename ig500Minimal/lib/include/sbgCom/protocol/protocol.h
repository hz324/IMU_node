...
#define SBG_MAX_DATA_LENGTH					(504)							/*!< Maximum size of the data part of the frame. */

#define SBG_SYNC							(0xFF)							/*!< First SYNC char of our frame. */
#define SBG_STX								(0x02)							/*!< Second STX char of our frame. */
#define SBG_ETC								(0x03)							/*!< Last ETC char of our frame. */
...
typedef struct _SbgProtocolHandleInt
{
	...
	uint16 serialBufferSize;							/*!< Size of our reception buffer */
	SbgDeviceHandle serialHandle;						/*!< Handle to our device */
...
/*!
 *	Handle type used by our protocol system.
 */
...
/*!
 *	Init our protocol system used to communicate with the product and return the created handle.
 ...
 *	\param[out]	pHandle					Pointer used to handle our created sbgCom library handle.
 *	\return								SBG_NO_ERROR if we have initialised our protocol system.
 */
SbgErrorCode sbgProtocolInit(const char *deviceName, uint32 baudRate, SbgProtocolHandle *pHandle);

/*!
 *	Close our protocol system and release associated memory.
 *	\param[in]	handle					A valid sbgCom library handle to close.
 *	\return								SBG_NO_ERROR if we have closed and released our protocol system.
 */
...
/*!
 *	Flush all data and our serial com port.
 *	\param[in]	handle		A valid sbgCom library handle.
 *	\return					SBG_NO_ERROR if our data has been flused.
 */
SbgErrorCode sbgProtocolFlush(SbgProtocolHandle handle);

/*!
 *	Change the baud rate used to communication with our device.<br>
 *	\param[in]	handle					A valid sbgCom library handle.
...
/*!
 *	Send a frame to our device (size should be less than 504 bytes).
 *	\param[in]	handle					A valid sbgCom library handle.
 *	\param[in]	cmd						Command number to send.
 *	\param[in]	pData					Pointer to our data field to send.
 *	\param[in]	size					Size of our data field to send.
 *	\return								SBG_NO_ERROR if our frame has been sent.
 */
SbgErrorCode sbgProtocolSend(SbgProtocolHandle handle, uint8 cmd, const void *pData, uint16 size);

/*!
 *	Try to receive a frame from our device and returns the cmd, data and size of data field (maxSize less than 504 bytes)
 *	\param[in]	handle					A valid sbgCom library handle.
 *	\param[out]	pCmd					Pointer to hold our returned command.
 *	\param[out]	pData					Allocated buffer used to hold received data field.
 *	\param[out]	pSize					Pointer used to hold our received data field size.
 *	\param[in]	maxSize					Max number of bytes that can be stored in our pData buffer.
 ...
 *	\param[out]	size					Size of our received data field buffer.
 *	\return								SBG_NO_ERROR if we have received a valid frame.
 */
 ...
 *	\param[in]	handle		A valid sbgCom library handle.
 *	\return					SBG_NO_ERROR if all our frames has been read sucessfully.
 */
 ...
 *	\param[in]	handle						A valid sbgCom library handle.
 *	\param[in]	callback					Pointer to our Triggered frame handler function.
 ...
 *	\param[in]	handle						A valid sbgCom library handle.
 *	\param[in]	callback					Pointer to our error handler function.
 ...
 *	\param[in]	handle						A valid sbgCom library handle.
 *	\param[in]	callback					Pointer to our Continuous frame handler function.
