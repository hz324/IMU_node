...
#define SBG_OUTPUT_TIME_SINCE_RESET				(0x00000800)			/*!< Enable our time since the device is on in milliseconds */

#define SBG_OUTPUT_DEVICE_STATUS				(0x00001000)			/*!< Enable the Device status output */

#define SBG_OUTPUT_GPS_POSITION					(0x00002000)			/*!< Enable our raw GPS position data */
#define SBG_OUTPUT_GPS_NAVIGATION				(0x00004000)			/*!< Enable our raw GPS navigation data such as velocity, heading */
#define SBG_OUTPUT_GPS_ACCURACY					(0x00008000)			/*!< Enable our raw GPS accuracy for both position and navigation data */
#define SBG_OUTPUT_GPS_INFO						(0x00010000)			/*!< Enable our raw GPS information data such as number of used space vehicles */

#define SBG_OUTPUT_BARO_ALTITUDE				(0x00020000)			/*!< Enable our barometric altitude referenced on a user defined reference pressure */
#define SBG_OUTPUT_BARO_PRESSURE				(0x00040000)			/*!< Enable our raw pressure output in pascal */

#define SBG_OUTPUT_POSITION						(0x00080000)			/*!< Enable our Kalman Enhanced 3d position in NED */
#define SBG_OUTPUT_VELOCITY						(0x00100000)			/*!< Enable our Kalman Enhanced 3d veclotiy expressed in device space (vX,vY,vZ) */

#define SBG_OUTPUT_ATTITUDE_ACCURACY			(0x00200000)			/*!< Enable our Kalman estimated attitude accuracy */
#define SBG_OUTPUT_NAV_ACCURACY					(0x00400000)			/*!< Enable our Kalman estimated position and velocity accuracy */
...
#define SBG_UTC_FINE_ACURACY_MASK				(0x00080000)					/*!< Bit mask for UTC time synchronized with our GPS  */
...
{
	uint32	outputMask;				/*!< Mask that validate our output field */
	float	stateQuat[4];			/*!< Our orientation quaternion w,x,y,z */
	float	stateEuler[3];			/*!< Our orientation in euler angles roll, pitch, yaw in radians */
	float	stateMatrix[9];			/*!< Our orientation 3x3 matrix stored like this:<br> */
									/*!< aX, aY, aZ, bX, bY, bZ, cX, cY, cZ */
	
	float	gyroscopes[3];			/*!< Calibrated version of our 3 gyroscopes, gX, gY, gZ */
	float	accelerometers[3];		/*!< Calibrated version of our 3 accelerometers, aX, aY, aZ */
	float	magnetometers[3];		/*!< Calibrated version of our 3 magnetometers, mX, mY, mZ */
	float	temperatures[2];		/*!< Calibrated temperature values, t0, t1 */
	
	uint16	gyroscopesRaw[3];		/*!< Raw values of our 3 gyroscopes */
	uint16	accelerometersRaw[3];	/*!< Raw values of our 3 accelerometers */
	uint16	magnetometersRaw[3];	/*!< Raw values of our 3 magnetometers */
	...
	float	attitudeAccuracy;		/*!< Attitude accuracy in rad estimated by our Kalman Filter */
	float	positionAccuracy;		/*!< Position accuracy in meters estimated by our Kalman Filter */
	float	velocityAccuracy;		/*!< Velocity accuracy in m/s estimated by our Kalman Filter */
...
/*!
 *	Fill our SbgOutput struct from a target based formatte raw buffer.<br>
 *	\param[in]	targetOutputMode		The output mode used by our target.<br>
 ...
 *	\param[in]	outputMask				Mask combinaison that defines which outputs are contained in our pBuffer.
 *	\param[in]	pBuffer					Raw buffer that contains our data to extract into SbgOutput.
 *	\param[in]	bufferSize				The size of our pBuffer.
 ...
 *	\param[in]	targetOutputMode		The output mode used by our target.<br>
 *										Defines if our data are stored in Big/Little endian <br>
 *										and if real values are stored in float or in fixed format.
 *	\param[in]	outputMask				Mask combinaison that defines which outputs are contained in our pBuffer.
